...
-------
## Introduction
cLoops2 is an extension of our previous work, [cLoops](https://github.com/YaqiangCao/cLoops). From loop-calling based on assumption-free clustering to a full suite of analysis tools for 3D genomic interactions data, cLoops2 has been adopted specifically for data such as Hi-Trac/Trac-looping, for which interactions are enriched over the genome through experimental steps. cLoops2 still supports Hi-C -like data, of which the interaction signals are evenly distributed at enzyme cutting sites.  The changes from cLoops to cLoops2 are designed to address challenges around aiming for higher resolutions with the next-generation of genome architecture mapping technologies. 

If you use cLoops2 in your research (the idea, the algorithm, the analysis scripts or the supplemental data), please give us a star on GitHub repo page and cite our paper as follows:    
...
-------
## Install
cLoops2 is written purely in Python3 (cLoops was written in Python2). If you are familiar with [conda](https://docs.conda.io/en/latest/), cLoops2 can be installed easily with the following Linux shell commands. 
```
git clone https://github.com/YaqiangCao/cLoops2
cd cLoops2
conda env create --name cLoops2 --file cLoops2_env.yaml
conda activate cLoops2 
python3 setup.py install
```
Then whenever you want to run cLoops2, just activate the environment with conda: **conda activate cLoops2**. 
Happy peak/loop-calling and have fun exploring all the other kinds of analyses.     

Necessary Python3 third-party packages are listed below:
```
tqdm
numpy 
scipy 
...
Example data for testing is available at [cLoops2/example/data](https://github.com/YaqiangCao/cLoops2/blob/master/example/data). The BEDPE files were from TrAC-looping experiments mapped to hg38 for chromosome 21 in GM12878 cells. Only intra-chromosomal PETs were kept. Raw FASTQ reads were processed by [tracPre.py](https://github.com/YaqiangCao/cLoops2/blob/master/scripts/tracPre.py).      
Due to GitHub's file size limitation, the example data is splited as 2 separate files. For other kinds of data such as ChIA-PET, Hi-C and HiChIP, cLoops2 can also start with provided BEDPE files.     
...
1. Get basic statistics of PETs from input BEDPE file
```
cLoops2 qc -f ../data/GM_Trac1_hg38_chr21_partaa.bedpe.gz,../data/GM_Trac1_hg38_chr21_partab.bedpe.gz -o test -p 2
```
The output is a .txt file with annotation of information as follows.   
...
|----------------------------|-----------|------------|-------------|---------------------------|-------------|-----------------------------|-------------|--------------|--------------------------|-------------|--------------------------------|-------------|---------------------------|-------------|
| GM_Trac1_hg38_chr21_partaa | 1640464   | 921709     | 0.438141282 | 712844                    | 0.773393772 | 208865                      | 0.226606228 | 201129.8101  | 504373                   | 0.70755032  | 109829                         | 0.154071578 | 98642                     | 0.138378102 | 
| GM_Trac1_hg38_chr21_partab | 1639216   | 619359     | 0.622161448 | 517507                    | 0.835552563 | 101852                      | 0.164447437 | 123567.7242  | 413619                   | 0.799252957 | 58037                          | 0.112147275 | 45851                     | 0.088599768 | 

2. Pre-process BEDPE file(s) into cLoops2 data    
The replicates are supplied as a comma-separated list. The output directory contains one [.json](https://github.com/YaqiangCao/cLoops2/blob/master/example/GM_Trac1/petMeta.json) file for the basic PET statistics and [.ixy](#.ixy) files, which are used to call peaks, loops or any analysis implemented in cLoops2.        
For data backup/sharing purposes, the directory can be saved as a .tar.gz file through ***tar*** command.   
```
cLoops2 pre -f GM_Trac1_hg38_chr21_partaa.bedpe.gz,GM_Trac1_hg38_chr21_partab.bedpe.gz -o GM_Trac1 -c chr21
```

3. Estimate reasonable contact matrix resolution    
This step is not needed for peak-calling 1D data such as ChIP-seq or ChIC-seq.    
We prefer to use the highest resolution with >=50% PETs (solid line) for filtering singletons, visualization or calling loops.         
```
cLoops2 estRes -d test -o test -bs 10000,5000,1000,200  
```
<p align="center">
<img align="center" width="400" height="275" src="https://github.com/YaqiangCao/cLoops2/blob/master/pngs/GM_Trac1_estRes.png">
</p>   

4. Filter singleton PETs
This step is not needed for 1D data like ChIP-seq/ChIC-seq for peak-calling.  The bin size can be estimated through ***cLoops2 estRes*** or experience.  
```
cLoops2 filterPETs -d test -o test_filtered -singleton -bs 1000
```

5. Call peaks    
Call peaks can be run on raw data or filtered data.  
```
cLoops2 callPeaks -d GM_Trac1 -o GM_Trac1 -eps 100 -minPts 10
```

6. Show aggregated peaks 
```
cLoops2 agg -d GM_Trac1 -peaks GM_Trac1_peaks.bed -o GM_Trac1 -peak_ext 2500 -peak_bins 200 -peak_norm
```
<p align="center">
<img align="center" width="200" height="500" src="https://github.com/YaqiangCao/cLoops2/blob/master/pngs/GM_Trac1_aggPeaks.png">
</p>

7. Call intra-chromosomal loops     
Call loops can be run on raw data or filtered data.    
```
cLoops2 call-cis-loops -d GM_Trac1 -o GM_Trac1 -eps 500,1000,2000 -minPts 3,5,10 -filter -cut 1000 -w -j -max_cut  
```

8. Show aggregated loops 
```
cLoops2 agg  -d GM_Trac1 -o GM_Trac1 -loops GM_Trac1_loops.txt -1D -loop_norm -bws GM12878_ATAC_chr21.bw,GM12878_CTCF_chr21.bw
```
<p align="center">
<img align="center" width="300" height="500" src="https://github.com/YaqiangCao/cLoops2/blob/master/pngs/GM_Trac1_aggLoops.png">
</p>

9. Call domains 

10. Show aggregated domains

11. Show interaction example   
cLoops2 can show the interaction contact matrix at any resolution.   

a.Show domains      
```
#show the chromsome with domain such as TAD
plotMatrix.py -f GM_Trac1/chr21-chr21.ixy -o GM_Trac1_domain_example -bs 5000 -start 34600000 -end 35500000 -domain GM_HiC_TAD_chr21.bed -log -triu -bw GM12878_CTCF_chr21.bw 
```
<p align="center">
<img align="center" width="400" height="500" src="https://github.com/YaqiangCao/cLoops2/blob/master/pngs/GM_Trac1_domain_example_matrix.png">
</p>    

b.Show loops         
```
#show enhancer-promoter loops
plotMatrix.py -f GM_Trac1/chr21-chr21.ixy -o GM_Trac1_example -bs 500 -start 33190000 -end 33280000 -triu -bw GM12878_ATAC_chr21.bw,GM12878_CTCF_chr21.bw -log -1D -loop GM_Trac1_loops.txt -beds GM12878_RoadMap_hg38_Enh_chr21.bed,GM12878_RoadMap_hg38_Tss_chr21.bed    
```    
<p align="center">
<img align="center" width="350" height="700" src="https://github.com/YaqiangCao/cLoops2/blob/master/pngs/GM_Trac1_example_matrix.png">
</p>


12. Call differential enriched loops   
```
Not well estabilished yet. 
```

13. Compare interactions and loops
```
Not well estabilished yet. 
```
...
Available sub-commands are: 
    ...
    plot: plot the interaction matrix, genes, virtual 4C view point plot, 1D 
          tracks, peaks, loops and domains for a specific region. 
...
------
### 9. Call peaks for 1D or 3D data
...
Examples:
    ...
    5. call inter-chromosomal loops (for most data, there will be no significant 
       inter-chromosomal loops)
        cLoops2 callLoops -d HiC -eps 5000 -minPts 10,20,50,100,200 -w -j -trans\                          -o HiC_trans
...
------
### 11. Call differentially enriched intra-chromosomal loops
...
Example:
    cLoops2 callDiffLoops -tloop target_loop.txt -cloop control_loop.txt -td ./target -cd ./control -o target_diff
...
optional arguments:
  ...
  -fdr FDR      FDR cutoff for estimating fold change (M) and averge value (A) after
...
------
### 13. Plot the interaction heatmap, 1D signals, peaks, loops and domains
Run **cLoops2 plot -h** to see details.
```
Plot the interaction data as a heatmap with additional of virtual 4C view point,
1D tracks (bigWig files), 1D annotations (peaks, genes) and 2D annotations 
(domains).
...
optional arguments:
  ...
  -triu                 Whether to rotate the heatmap only show upper triangle, default is
                        False.
  -1D                   Whether to plot the pileup 1D signal. Default is not.
...
------
### 14. Montage analysis for regions of interactions
...
optional arguments:
  ...
  -vmax VMAX     The maxmial scale for 1D pileup data. Default will be inferred from the
                 data.
  -ppmw PPMW     Link line width indicator, short for 1 PETs per Million PETs linke
...
------
### 15. Aggregation analysis for peaks, loops and domains
...
For the aggregated virtual 4C analysis, aka aggregated view points analysis, 
input is a .bed file annotated with coordinates for the target 
regions/peaks/anchors as view point. Output is a .pdf file containing a mean 
density plot and heatmap and a .txt file for the data. The data in the .txt 
file and plot were normalized to log2( RPM (reads per million)+1).
...
Examples:
    ...
    2. show aggregated loops heatmap, 1D profile and aggregated bigWig signal
        cLoops2 agg -d test -o test -loops test_loops.txt -bws CTCF.bw -1D \
                    -loop_norm
    
    3. show aggregated view points and aggregated bigWig signal
        cLoops2 agg -d test -o test -viewPoints test_peaks.txt -bws CTCF.bw
...
optional arguments:
  ...
  -viewPoint_upExt VIEWPOINT_UPEXT
                        The upstream regions included for the aggreaged virtual 4C analysis.
                        Fold of the view point region size. Default is 10.
  -viewPoint_downExt VIEWPOINT_DOWNEXT
                        The downstream regions included for the aggreaged virtual 4C analysis.
                        Fold of the view point region size. Default is 10.
  ...
  -1D                   Whether to plot the pileup 1D signal for aggregated loops or
                        aggregated domains. Default is not.
...
------
### 16. Quantification of peaks, loops and domains
...
optional arguments:
  ...
  -domains DOMAINF      The .bed file annotated the domains such as TADs for 
                        domains-centric quantification.
...
------
------
## Analysis Application Scripts
The following analysis application scripts are available when cLoops2 is installed. The majority of them can be independently run. The -h option can show example usages and details of parameters. Some of them will be integrated into cLoops sub-programmes if well tested. 
...
- [findRich.py](https://github.com/YaqiangCao/cLoops2/blob/master/scripts/findRich.py): find the regulatory interaction chromatin hub (rich) regions based on rich score for Trac-looping data. The rich score is modified from the [Directionality Index](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3356448/) as <img align="center" src="https://latex.codecogs.com/svg.latex?\Large&space;S_{x}=\frac{(A-B)^2}{A+B}"/>,where **A** is the number of all reads interacted with upstream for the bin **x** and **B** is the number of reads interacted with downstream, no distance limitation is needed. The final score was normalized to the mean score for the chromosome. Together with normalized interaction score (the interaction score is defined as the reads interacting with the bin, also normalized to mean score for the chromosome), the rich regions are called. Compared to the directionality index and insulation score, the rich score does not require the parameter of the region size. It is more suitable for the unevenly distributed interaction data, and the bin size can be smaller. 
...
- [tracPre.py](https://github.com/YaqiangCao/cLoops2/blob/master/scripts/tracPre.py): pre-process the raw reads of FASTQ files of Trac-looping to the reference genome and obtain the unique PETs with quality control results.

---
### Analysis with plot
- [getObsExpDisFreq.py](https://github.com/YaqiangCao/cLoops2/blob/master/scripts/getObsExpDisFreq.py): get the observed/expected genomic interaction frequency vs genomic distances, then do the log-log plot. For example the Trac-looping data:
```
getObsExpDisFreq.py -d ./GM_Trac1 -o GM_Trac1 -r 10 -plot -p 20 
```  
The output data is as follows.     

|distance(bp) | observed | expected | Obs/Exp |
|----------|----------|---------| ------ |
| 0        | 897488   | 313     | 2867.373802 |
| 5000     | 106845   | 476     | 224.4642857 |
| 10000    | 21571    | 456     | 47.30482456 |
| 15000    | 11673    | 458     | 25.48689956 |
| ...    | ...    | ...     | ... |

The output figure as following.     
<p align="center">
<img align="center" width="400" height="300" src="https://github.com/YaqiangCao/cLoops2/blob/master/pngs/GM_Trac1_obsExpDisFreq.png">
</p>

- [getSigDist.py](https://github.com/YaqiangCao/cLoops2/blob/master/scripts/getSigDist.py): get the observed/expected interaction signal distribution in contact matrix at any assigned resolution.

- [estSigEnrich.py](https://github.com/YaqiangCao/cLoops2/blob/master/scripts/getSigEnrich.py): get the similar plot to [ChIP-seq fingerprint plot](https://deeptools.readthedocs.io/en/develop/content/tools/plotFingerprint.html) for the interaction data, useful to check/compare the signal distribution/enrichment. For example the Trac-looping data:
```
getSigEnrich.py -d test -o test -r 5 -p 20 -plot -bs 1000
```

The output data as following.      

| bins     | observed    |  expected   | 
|----------|-------------|-------------| 
| 0 |	0.0008407030623749536 |	0.00032346813491055933 |
| 1 |	0.0016814061247499071 |	0.0006469362698211187 |
| 2 |	0.0025221091871248606 |	0.000970404404731678 |
| ...      | ... | ... | 

The output figure as following. The further from the expected line, the more enriched the signal, which may detect more significant loops ( such as peaks in 1D ChIP-seq data).     
<p align="center">
<img align="center" width="400" height="300" src="https://github.com/YaqiangCao/cLoops2/blob/master/pngs/GM_Trac1_sigEnrich.png">
</p>

- [plotAggRich.py](https://github.com/YaqiangCao/cLoops2/blob/master/scripts/plotAggRich.py): get the aggregated heatmap of target rich regions and nearby regions. Output will be a heatmap pdf and .npz file save the observed matrix for each rich region, can be load with [numpy.load](https://docs.scipy.org/doc/numpy/reference/generated/numpy.load.html). 
```
getAggRichPlot.py -d Trac -b Trac_RICH.bed -o Trac_rich -p 30 -ext 20000 -cut 1000 
```
...
6th | segregationScore | mean segregation score for all bins within the domain  
7th | totalPETs | number of total PETs in the domain
8th | withinDomainPETs | number of PETs only interacting within the domain
9th | enrichmentScore | (withinDomainPETs) / (totalPETs-withinDomainPETs)
10th | density | similarly to that of RPKM (reads per kilobase per million):<img align="center" src="https://latex.codecogs.com/svg.latex?\Large&space;density=\frac{withinDomainPETs}{(libraryTotalPETs)\times(domainLength)}\times10^9" />
